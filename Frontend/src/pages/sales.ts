...
import type { SaleResponse, SalesDetailCreate, CustomerResponse, ProductResponse, StorageResponse } from "../types";
...
function buildCreateForm(
  ...
  onSubmit: (data: { invoiceNumber: string; customerId: string; totalDiscount?: number; details: SalesDetailCreate[] }) => Promise<void>,
): HTMLElement {
  const wrapper = document.createElement("div");
  wrapper.innerHTML = `
    <form id="sale-form" class="space-y-4">
      <div>
        <label class="block text-sm font-medium text-gray-700 mb-1">Nro. Factura <span class="text-red-500">*</span></label>
        <input name="invoiceNumber" required maxlength="50" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" />
      </div>
      <div>
        <label class="block text-sm font-medium text-gray-700 mb-1">Cliente <span class="text-red-500">*</span></label>
        <select name="customerId" required class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
          <option value="">Seleccionar...</option>
          ${customers.map((c) => `<option value="${c.id}">${c.firstName} ${c.firstSurname} — ${c.documentIdNumber}</option>`).join("")}
        </select>
      </div>
      <div>
        <label class="block text-sm font-medium text-gray-700 mb-1">Descuento Total</label>
        <input name="totalDiscount" type="number" min="0" step="0.01" value="0" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" />
      </div>
      ...
      <button type="submit" class="w-full px-4 py-2 text-sm font-medium text-white bg-indigo-600 rounded-lg hover:bg-indigo-700 mt-2">Guardar Venta</button>
    </form>
  `;
  ...
  function addLine(): void {
    ...
    line.innerHTML = `
      <div class="col-span-2">
        ...
        <select name="detail_product_${idx}" required class="w-full px-2 py-1.5 border border-gray-300 rounded text-xs">
          <option value="">Seleccionar</option>
          ${products.map((p) => `<option value="${p.sku}" data-price="${p.salePrice}">${p.sku} — ${p.productName}</option>`).join("")}
        </select>
      </div>
      <div>
      ...
      <div>
        ...
        <input name="detail_price_${idx}" type="number" min="0" step="0.01" required class="w-full px-2 py-1.5 border border-gray-300 rounded text-xs" />
      </div>
    ...
    const priceInput = line.querySelector(`[name="detail_price_${idx}"]`) as HTMLInputElement;
  ...
  const form = wrapper.querySelector("#sale-form") as HTMLFormElement;
  form.addEventListener("submit", async (e) => {
    e.preventDefault();
    const fd = new FormData(form);
    const details: SalesDetailCreate[] = [];
    ...
    for (const line of lines) {
      ...
      const price = parseFloat(fd.get(`detail_price_${idx}`) as string);
    ...
    const totalDiscount = parseFloat(fd.get("totalDiscount") as string);
    await onSubmit({
...
function buildDetailView(sale: SaleResponse): HTMLElement {
  const el = document.createElement("div");
  el.innerHTML = `
    <div class="space-y-3 text-sm">
      <div class="grid grid-cols-2 gap-4 mb-4">
        <div><span class="text-gray-500">Factura:</span> <strong>${sale.invoiceNumber}</strong></div>
        <div><span class="text-gray-500">Cliente:</span> <strong>${sale.customerName}</strong></div>
        <div><span class="text-gray-500">Vendedor:</span> <strong>${sale.username}</strong></div>
        <div><span class="text-gray-500">Estado:</span> ${statusBadge(sale.status)}</div>
        <div><span class="text-gray-500">Total:</span> <strong>${formatCurrency(sale.totalAmount)}</strong></div>
        <div><span class="text-gray-500">Descuento:</span> ${formatCurrency(sale.totalDiscount ?? 0)}</div>
      </div>
        ...
        <tbody>
          ${sale.details.map((d) => `
            <tr class="border-t">
              ...
              <td class="px-3 py-2 text-right">${formatCurrency(d.unitPrice)}</td>
...
export async function salesPage(): Promise<void> {
  ...
  try {
    ...
    content.innerHTML = `
      <div class="flex items-center justify-between mb-6">
        ...
        <button id="create-btn" class="px-4 py-2 text-sm font-medium text-white bg-indigo-600 rounded-lg hover:bg-indigo-700">+ Nueva Venta</button>
      </div>
    ...
    renderTable(document.getElementById("table-container")!, columns, data, [
      {
        ...
        variant: "primary",
        onClick: async (item) => {
          const sale = await saleApi.getById(item.id);
          openModal({ title: `Venta ${sale.invoiceNumber}`, content: buildDetailView(sale) });
        },
      },
      {
        label: "Eliminar",
        variant: "danger",
        onClick: (item) => {
          confirmDialog("¿Eliminar esta venta?", async () => {
            try {
              await saleApi.remove(item.id);
    ...
    document.getElementById("create-btn")!.addEventListener("click", async () => {
      const [customers, products, storages] = await Promise.all([
        customerApi.getAll(),
        productApi.getAll(),
        storageApi.getAll(),
      ]);
      const formEl = buildCreateForm(customers, products, storages, async (formData) => {
        try {
      ...
      openModal({ title: "Nueva Venta", content: formEl });
    });
  } catch (err) {
